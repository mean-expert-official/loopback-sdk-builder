...
exports.describeModels = function describeModels(app) {
  ...
  app.handler('rest').adapter.getClasses().forEach(function (c) {
    ...
    if (c.sharedClass.ctor.settings.sdk.calculatedProperties) {
      for (var i in c.sharedClass.ctor.settings.sdk.calculatedProperties) {
        if (c.sharedClass.ctor.settings.sdk.calculatedProperties.hasOwnProperty(i)) {
          c.properties[i] = c.sharedClass.ctor.settings.sdk.calculatedProperties[i];
          
          // check if property type is an array
            if(Array.isArray(c.properties[i].type)){
            // check which value types are allowed for the array
            // this will be the string representation of the type
            // example: methodIds: { type: [ 'number' ] } } 
              let typeAsString = c.properties[i].type[0]; 
              c.properties[i].type = [getType(typeAsString)];
            } else {
              c.properties[i].type = getType(c.properties[i].type);
            }
        }
      }
    }
