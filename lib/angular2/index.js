...
module.exports = function generate(ctx) {
  ...
  **/
  ...
  function buildMethodParams(model, methodName, params, isIo) {
    ...
    let relations = getModelRelations(model);
    //if (model.name === 'Account') console.log(relations);
    ...
    relations.forEach(relationName => {
        ...
        availableClasses.push(capitalize(model.sharedClass.ctor.relations[relationName].modelThrough.sharedClass.name));
      }
    );
