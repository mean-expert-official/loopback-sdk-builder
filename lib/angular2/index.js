...
module.exports = function generate(ctx) {
  ...
  **/
  ...
  /** 
   * DEFAULT VALUES MODE : strict | ''
   * if 'strict' : only default values defined in loopback models will be set
   * change it to any other value to set default values for all models
   */
  ctx.defaultValueMode = 'strict';
  /**
   * SDK DYNAMIC FILES
   */
  ...
  function buildModelProperties(model, isInterface) {
    ...
    Object.keys(model.properties).forEach((property) => {
      ...
      let defaultValue = !isInterface && ctx.defaultValue === 'enabled' ? ` = ${buildPropertyDefaultValue(meta)}` : '';
      defaultValue = defaultValue && ( ctx.defaultValueMode === 'strict' && meta.hasOwnProperty('default') ) ? defaultValue : '';
    ...
    Object.keys(model.sharedClass.ctor.relations).forEach(relation => {
      let rel = `  ${relation}${isInterface ? '?' : ''}: ${buildRelationType(model, relation)}`;
      rel += `${( !isInterface && ctx.defaultValue === 'enabled' ) ? ' = null' : ''}`;
      rel += ';';
      output.push( rel );
    });
    return output.join('\n');
