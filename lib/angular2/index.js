...
module.exports = function generate(ctx) {
  ...
  let directories = [
    ...
    ctx.outputFolder + '/orm',
    ctx.outputFolder + '/orm/models'
  ];
  ...
  **/
  ...
  if (ctx.isNgrx === 'enabled' && ctx.isIo === 'enabled') {
    schema = schema.concat([
      {
      ...
      */
      if (ctx.isNgrx === 'enabled') {
        ...
        schema.push({
          ...
          output: '/orm/models/' + modelName + '.ts',
          params: {
            isIo: ctx.isIo,
            model: ctx.models[modelName],
            modelName: modelName,
