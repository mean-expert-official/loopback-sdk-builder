    ...
    Object.keys(ctx.models).forEach(modelName => {
      for (const rel in ctx.models[modelName].sharedClass.ctor.relations) {
        if (ctx.models[modelName].sharedClass.ctor.relations[rel].modelThrough) {
          const thoughModel = ctx.models[modelName].sharedClass.ctor.relations[rel].modelThrough.definition.name
          if (typeof ctx.models[thoughModel] === 'undefined' && !thoughModels.hasOwnProperty(thoughModel)) {
            thoughModels[thoughModel] = {
              from: modelName,
  ...
  **/
  let schema = [
    ...
    },
    {
      ...
      params: {
        isIo: ctx.isIo,
        models: Object.assign({}, ctx.models, thoughModels)
      }
    },
    {
  ...
  if (ctx.isNgrx === 'enabled' || ctx.isNgrx === 'orm') {
    schema = schema.concat([
       ...
       */
      {
        ...
        output: '/state.ts',
        params: { models: ctx.models, thoughModels }
      },
      {
      ...
      },
      {
        ...
        params: { models: Object.assign({}, ctx.models, thoughModels) }
      },
      /**
       * NGRX Effects
       */
      ...
      },
      {
        ...
        params: { models: Object.assign({}, ctx.models, thoughModels), isNgrx: ctx.isNgrx }
      },
      /**
       * NGRX Resolvers
       */
   ...
   */
  if (ctx.isNgrx === 'orm') {
    Object.keys(thoughModels).forEach(modelName => {
      schema.push(
        {
          template: './shared/models/throughModel.ejs',
          output: '/models/' + capitalize(modelName) + '.ts',
          params: {
            model: thoughModels[modelName],
            modelName: modelName
          }
        }
      );
