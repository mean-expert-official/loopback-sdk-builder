...
/**
 * Generate Nativescript2 $resource services for the given loopback application.
 *
 * ```js
 * var generateServices = require('loopback-sdk-builder').nativescript2;
 ...
 * @param {string=} ngModuleName A name for the generated Nativescript2 module.
 *   Default: `lbServices`.
 * @param {string=} apiUrl The URL where the client can access the LoopBack
 *   server app. Default: `/`.
 * @param {string=} framework Client's framework. Default: `nativescript2`.
...
module.exports = function generate(app, ngModuleName, apiUrl, outputFolder) {
  ...
  try {
    ...
    console.error(
      '\033[31mTemplate for "%s" not found\033[0m',
      'nativescript2'
    );
    process.exit(1);
  }
