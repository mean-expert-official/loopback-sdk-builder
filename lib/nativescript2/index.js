...
function generateHelperFiles(models, ngModuleName, apiUrl, outputFolder, isIo) {
  ...
  if (outputFolder) {
    outputFolder = path.resolve(outputFolder);
    fs.writeFileSync(outputFolder + '/services/logger.service.ts', result);
  }
  ...
  if (outputFolder) {
    outputFolder = path.resolve(outputFolder);
    fs.writeFileSync(outputFolder + '/services/logger.config.ts', result);
  }
  ...
  if (outputFolder) {
    outputFolder = path.resolve(outputFolder);
    fs.writeFileSync(outputFolder + '/services/search.params.ts', result);
  }
