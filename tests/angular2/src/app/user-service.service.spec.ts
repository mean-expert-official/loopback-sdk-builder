...
describe('UserService Service', () => {
  ...
  it('should create a new instance',
    injectAsync([UserApi], (userApi: UserApi) => {
      return userApi.create(user).subscribe((user: UserInterface) => expect(user.id).toBeTruthy());
    })
  );

  it('should login the user',
    injectAsync([UserApi], (userApi: UserApi) => {
      return userApi.login(user).subscribe((token: TokenInterface) => expect(token.userId).toBeTruthy());
    })
  );
